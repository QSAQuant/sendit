...
/** Casino-handle style names — feels like a live room, not NPC1 */
const BOT_HANDLES = [
  "rio.br",
  "nova_x",
  "mule.gg",
...
function stakeFromRng(rng: () => number): number {
  const roll = rng();
  // Heavy micro / mid, rare whale — reads as a real lobby
...
export function makeYou(name: string, ridge: RidgeId, bet: number): Actor {
  return {
    id: "you",
    name,
    you: true,
